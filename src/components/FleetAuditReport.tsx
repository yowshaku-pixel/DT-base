...
export const FleetAuditReport: React.FC<FleetAuditReportProps> = ({ records, fleetRegistry, onFocusTruck, onRefresh, isRefreshing }) => {
  ...
  const auditData = useMemo(() => {
    ...
    const folders: string[] = cleanRegistry.length > 0 ? cleanRegistry.sort() : Object.keys(groups).sort();
    
    // De-duplicate folder names just in case registry contains similar plates that normalized to same thing
    const uniqueFolders: string[] = Array.from(new Set(folders));
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          {onRefresh && (
            <button 
              ...
              className={cn(
                "w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2.5 bg-surface border border-border rounded-xl text-[10px] font-display font-bold text-text uppercase tracking-widest hover:bg-bg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 transition-all cursor-pointer",
          ...
          <div className="relative group w-full md:w-80">
            <label htmlFor="fleet-audit-search" className="sr-only">Search fleet audit records by plate number</label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {auditData.map((truck) => (
          <div 
            ...
            aria-label={`View maintenance logs for ${truck.plate}`}
            className="group relative bg-surface border border-border rounded-3xl overflow-hidden hover:border-purple-500/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 transition-all text-left cursor-pointer"
          >
            <div className="p-4 sm:p-5">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-1 text-purple-400 opacity-0 group-hover:opacity-100 group-focus-visible:opacity-100 transition-opacity">
