    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-surface border border-border rounded-[2rem] p-6 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-24 h-24 bg-purple-500/5 -mr-8 -mt-8" />
        ...
        <div className="bg-surface border border-border rounded-[2rem] p-6 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-24 h-24 bg-red-500/5 -mr-8 -mt-8" />
        ...
        <div className="bg-surface border border-border rounded-[2rem] p-6 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-24 h-24 bg-amber-500/5 -mr-8 -mt-8" />
          <div className="flex items-center gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {batteryStats.analysis.map((truck) => (
          <div 
            key={truck.plate}
            className={cn(
              "p-4 sm:p-5 rounded-3xl border transition-all duration-500 group",
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className={cn(
                  "w-11 h-11 rounded-2xl flex items-center justify-center border transition-transform group-hover:scale-110",
                  truck.status === 'critical' ? "bg-red-500/10 border-red-500/20" : 
                  truck.status === 'warning' ? "bg-amber-500/10 border-amber-500/20" : 
                  "bg-bg border-border"
                )}>
                  {truck.health > 80 ? <BatteryFull className="w-5 h-5 text-green-500" /> : 
                   truck.health > 40 ? <BatteryMedium className="w-5 h-5 text-amber-500" /> : 
                   <BatteryLow className="w-5 h-5 text-red-500" />}
                </div>
                <div>
                  <h4 className="text-base font-display font-bold text-text tracking-tight">{truck.plate}</h4>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex items-center gap-1">
                      <Zap className="w-2.5 h-2.5 text-purple-400" />
                      <span className="text-[9px] font-mono text-muted uppercase">{truck.repairCount} Repairs</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="text-right">
                ...
                <p className="text-[8px] font-mono text-muted uppercase tracking-widest">Health</p>
              </div>
            </div>

            <div className="space-y-3">
              <div className="p-3 bg-bg/50 rounded-xl border border-white/5">
                <div className="flex items-center gap-2 mb-1">
                  <TrendingDown className={cn(
                    "w-3 h-3",
                    truck.status === 'critical' ? "text-red-400" : "text-purple-400"
                  )} />
                  <span className="text-[8px] font-display font-bold uppercase tracking-widest opacity-40">Insight</span>
                </div>
                <p className={cn(
      ...
      <div className="p-8 text-center bg-surface border border-border border-dashed rounded-[2rem]">
        <div className="inline-flex items-center gap-3 px-4 py-2 bg-surface rounded-full border border-border mb-4">
          <Zap className="w-3.5 h-3.5 text-purple-400" strokeWidth={3} />
